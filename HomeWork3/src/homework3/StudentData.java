...
public class StudentData {
     ...
     while(year<23){ // year < 23 
         for(int i=0;i<faculty.length;i++) {
             for(int j=0;j<department.length;j++) {
                 ...
                 std = new Student(id,female.getNextName(),lastName.getNextName(),faculty[i],department[j]) ; 
                 }
                 else  {  // generate male student 
                 std = new Student(id,male.getNextName(),lastName.getNextName(),faculty[i],department[j]) ; 
                 }
    ...
    public int createID(int year,int fac,int dep) {
        ...
        // if random method gives same number for same department students, code snip will change it
