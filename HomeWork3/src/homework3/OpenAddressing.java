...
public class OpenAddressing extends Hashing {
    ...
    public OpenAddressing(StudentData data) {
       ...
       // Fetch the students from the ArrayList, then hash and allocate them into the table you created.
        ...
        System.out.println("\n(part a)Fetch the students from the ArrayList, then hash and allocate them into the table we created in "+timedif()+" milliseconds\n");
        System.out.println("-----------------------------------------------------------------------------------------------------------------------------------------");
...
// part b -------- Search and bring hundred random students from table----------
          ...
          for(int count =0 ;count<100 ;count++) {
            ...
            while(set.contains(set))  index_std = (int)(Math.random()*81000) ; 
            set.add(index_std) ;
            ...
            for(int i =0 ;i<tableSize;i++) {  // probing i={0,1,2,...tableSize-1} 
               ...
               if(table[index] == null) break;   // checking slot is null or not 
        ...
        System.out.println("\n(part b)Search and bring hundred random students from the table in "+timedif()+" milliseconds.\n\n");
...
//------------------- double the table if it is full ---------------------------    
    private void tableDoubling() {
     ...
     //  System.out.println("TableDuoble prime : "+tableSize+" size : "+size) ;
       ...
       for(int k =0;k<table.length;k++) {   // move old table to new table 
           for(int i = 0;i<table.length;i++) {   // probing ith for values {0,1,2, ... ,tableSize-1 } but funcProb uses new tablesize
    ...
    // ---- return the smallest prime number bigger than the twice of the size of the table ---- 
